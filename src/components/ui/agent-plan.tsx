...
export interface PlanProps {
  /** When provided, seeds the component's task list instead of the built-in
   *  demo data. Toggling status/expansion stays local (same UX as demo). */
  tasks?: Task[];
}
...
export default function Plan({ tasks: tasksProp }: PlanProps = {}) {
  const [tasks, setTasks] = useState<Task[]>(tasksProp ?? initialTasks);
  // When a parent owns the task list (e.g. ConversationTodos driven by
  // `todo_write`), sync local state whenever the prop reference changes.
  // Without this, updates only appear after a remount — switching
  // conversations away and back — because useState seeds once.
  useEffect(() => {
    if (tasksProp !== undefined) setTasks(tasksProp);
  }, [tasksProp]);
  const [expandedTasks, setExpandedTasks] = useState<string[]>(() => {
    const first = (tasksProp ?? initialTasks)[0]?.id;
    return first ? [first] : [];
  });
  ...
  // When the user is selecting text (to copy log output), the mouseup that
  // ends the selection would otherwise bubble into `onClick` and collapse
  // the block. Skip the toggle if there's a non-empty selection.
